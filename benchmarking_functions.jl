...
function get_energies(geometry::String, Ls::Vector{Int64}, N::Int64, bcs::Vector{String}, U::Float64)
    ...
    V0 = sum([BH_interaction(configs[i], U)*Ψ0[i]^2 for i=1:length(configs)])
    K0 = E0 - V0
...
function plot_beta(geometry::String, Ls::Vector{Int64}, N::Int64, bcs::Vector{String}, U::Float64, val::String; i=1)
    vals = Dict{String,Tuple{String,String}}(
        "E"=>(L"$\langle E\:\rangle$","Energy"),
        "K"=>(L"$\langle K\:\rangle$","Kinetic Energy"),
        "V"=>(L"$\langle V\:\rangle$","Potential Energy"),
        "n"=>(L"$\langle n_i \rangle$","Boson Density"))
    ...
    data = [QMC_data(folderpath * "/" * b) for b in betas]
    betas = parse.(Float64, [b[1:findlast(isdigit, b)] for b in betas])
    M = get_M(geometry, Ls)
    if val != "n"
        y  = [d[val] for d in data if val in keys(d)]
        print(val, ": ", y, "\n\n")
        Δy = [QMC_error(d)[val] for d in data if val in keys(d)]
        ED = get_energies(geometry, Ls, N, bcs, U)[val]
        hline([ED], lw=2, seriescolor="red", label="ED Value")
        Plots.plot!(betas, y, lw=2, ls=:dash, primary=false, seriescolor="red")
        Plots.plot!(betas, y; yerror=Δy, seriestype=:scatter, primary=false, mw=1, seriescolor="red")
        Plots.plot!(title="Ground State "*vals[val][2]*"\n($(geometry), M=$(M), N=$(N), U/t=$(U), $(_bc_label(bcs)))\n")
        Plots.plot!(xlabel=L"$\beta$", ylabel=vals[val][1], legend=:right, xticks=betas,
            margin=0.5mm, plot_titlefontsize=12, legendfontsize=10, xlabelfontsize=12, tickfontsize=9)
    elseif val == "n"
        y1  = [d[val][i] for d in data if val in keys(d)]
        print("n$(i): ", y1, "\n\n")
        Δy1 = [QMC_error(d)[val][i] for d in data if val in keys(d)]
        ED  = get_densities(geometry, Ls, N, bcs, U)[val]
        hline([ED[i]], lw=2, seriescolor="red", label="ED Value (i=$(i))")
        Plots.plot!(xlabel=L"$\beta$", ylabel=vals[val][1], xticks=betas)
        Plots.plot!(betas, y1, lw=2, ls=:dash, primary=false, seriescolor="red")
        Plots.plot!(betas, y1; yerror=Δy1, seriestype=:scatter, primary=false, mw=1, seriescolor="red")
        Plots.plot!(title="Ground State "*vals[val][2]*"\n($(geometry), M=$(M), N=$(N), U/t=$(U), $(_bc_label(bcs)))\n")
    end
end

function plot_rel_error(geometry::String, Ls::Vector{Int64}, N::Int64, bcs::Vector{String}, U::Float64, val::String)
    vals = Dict{String,Tuple{String,String}}(
        "E"  =>(L"$\langle E_0 \rangle$","Energy"),
        "K"  =>(L"$\langle K_0 \rangle$","Kinetic Energy"),
        "V"  =>(L"$\langle V_0 \rangle$","Potential Energy"),
        "n"  =>(L"$\langle n_i \rangle$","Boson Density"),
        "n^2"=>(L"$\langle n^2_i \rangle$","Boson Density Squared"),
        "σ^2_n"=>(L"\langle \sigma^2_{n_i} \rangle","Boson Density Variance"))
    folderpath = _sim_folder(geometry, Ls, N, bcs, U)
    betas = filter(!=(".DS_Store"), readdir("/Users/liamjones/Pigsfli.jl/out/" * folderpath))
    sort!(betas, by=length)
    data  = [QMC_data(folderpath * "/" * b) for b in betas]
    ...
    if val != "n"
        ED = get_energies(geometry, Ls, N, bcs, U)[val]
        E  = [d[val] for d in data if val in keys(d)]
        y  = [abs(e - ED) / abs(ED) for e in E]
        print("Δ$(val): ", y, "\n\n")
        Δy = [QMC_error(d)[val] / abs(ED) for d in data if val in keys(d)]
        Plots.plot(betas, y, lw=2, ls=:dash, primary=false, seriescolor="green")
        Plots.plot!(betas, y; yerror=Δy, seriestype=:scatter, primary=false, mw=1, seriescolor="green")
        Plots.plot!(title="Relative Error for Ground State "*vals[val][2]*"\n(M=$(M), N=$(N), U/t=$(U), $(_bc_label(bcs)))\n")
        Plots.plot!(xlabel=L"$\beta$", ylabel="Relative Error", xticks=betas,
            margin=0.5mm, plot_titlefontsize=12, legendfontsize=10, xlabelfontsize=12, tickfontsize=9)
    elseif val == "n"
        ED = get_densities(geometry, Ls, N, bcs, U)
        i1, i2 = 1, M÷2
        n1  = ED[val][i1];  n2  = ED[val][i2]
        y1  = [abs(d[val][1]  - n1) / n1 for d in data if val in keys(d)]
        print("Δn$(i1): ", y1, "\n\n")
        Δy1 = [QMC_error(d)[val][1]  / n1 for d in data if val in keys(d)]
        y2  = [abs(d[val][i2] - n2) / n2 for d in data if val in keys(d)]
        print("Δn$(i2): ", y2, "\n\n")
        Δy2 = [QMC_error(d)[val][i2] / n2 for d in data if val in keys(d)]
        Plots.plot(betas, y1, lw=2, ls=:dash, primary=false, seriescolor="green")
        Plots.plot!(betas, y1; yerror=Δy1, seriestype=:scatter, label="i=$(i1)", mw=1, seriescolor="green")
        Plots.plot!(betas, y2, lw=2, ls=:dash, primary=false, seriescolor="turquoise")
        Plots.plot!(betas, y2; yerror=Δy2, seriestype=:scatter, label="i=$(i2)", mw=1, seriescolor="turquoise")
        Plots.plot!(title="Relative Error for Ground State "*vals[val][2]*"\n(M=$(M), N=$(N), U/t=$(U), $(_bc_label(bcs)))\n")
        Plots.plot!(xlabel=L"$\beta$", ylabel="Relative Error", legend=:right, xticks=betas,
            margin=0.5mm, plot_titlefontsize=12, legendfontsize=10, xlabelfontsize=12, tickfontsize=9)
    end
end
...
function plot_corr_mat(data::Dict{String, Any})
    ...
    indices = CartesianIndices(mat)
    x = [indices[i][1] for i=1:length(indices)]
    y = [indices[i][2] for i=1:length(indices)]
    z = [(0.4,0.4,data[indices[i]]) for i=1:length(indices)]
    err = [(0.4,0.4,abs(data[indices[i]]-mat[indices[i]])/mat[indices[i]]) for i=1:length(indices)]
    ...
    linesegments!(f[1,1],[x[i] for i=1:length(indices) for t=1:2],[y[i] for i=1:length(indices) for t=1:2],[z[i][3]+t*stat_error[indices[i]]*bar for i=1:length(indices) for t in [1,-1]],color="black",linewidth=1,linecap=:square,fxaa=true)
    linesegments!(f[1,1],[x[i]+t*0.08 for i=1:length(indices) for t in [1,-1]],[y[i]+t*0.08 for i=1:length(indices) for t in [-1,1]],[z[i][3]+stat_error[indices[i]]*bar for i=1:length(indices) for t=1:2],color="black",linewidth=2,linecap=:square,fxaa=false)
    meshscatter!(f[1,1],x,y,markersize=z,marker = Rect3f((-0.5, -0.5, 0), (1, 1, 1)),color=[t[3] for t in z])
    Colorbar(f[1,2],limits=(minimum(data),maximum(data)),colormap=:viridis,flipaxis=false,vertical=true,size=10)
    
    f[1,3] = Axis3(f,xlabel=L"
    ",ylabel=L"
    ",zlabel="",aspect=(12,12,12),title="Relative Error",titlefont=:regular,titlesize=18,elevation=e,perspectiveness=p,azimuth=a,protrusions=s,xlabeloffset=30,ylabeloffset=30,zlabeloffset=40,xlabelsize=18,ylabelsize=18,zlabelsize=18)
    meshscatter!(f[1,3],x,y,markersize=err,marker = Rect3f((-0.5, -0.5, 0), (1, 1, 1)),color=[t[3] for t in err],colormap=:acton)
    Colorbar(f[1,4],colormap=:acton,flipaxis=false,halign=:left,limits=(minimum(errors),maximum(errors)),vertical=true,size=10)
    
    resize_to_layout!(f)
    f
end

plot_corr_mat(geometry::String, Ls::Vector{Int64}, N::Int64, bcs::Vector{String}, U::Float64, beta::Float64) = plot_corr_mat(QMC_data(_sim_path(geometry, Ls, N, bcs, U, beta)))
...
function plot_density_2D(data::Dict{String,Any})
    ...
    M       = get_M(geometry, Ls)
    lattice = get_lattice(geometry, Ls, bcs)
    x       = [lattice[i].x for i=1:length(lattice)]
    y       = [lattice[i].y for i=1:length(lattice)]
    ...
    err = [abs((ED[i] - density[i])) / ED[i] for i=1:length(density)]
...
plot_density_2D(geometry::String, Ls::Vector{Int64}, N::Int64, bcs::Vector{String}, U::Float64, beta::Float64) = plot_density_2D(geometry, Ls, N, bcs, QMC_data(_sim_path(geometry, Ls, N, bcs, U, beta)))
